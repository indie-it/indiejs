...
Router.route('/profile/view/:_id', {
	...
	data: function () {
		...
		var account = Meteor.users.findOne({ _id: profile.userid });
		console.log(account);
	...
	waitOn: function () {
		...
		return [
			Meteor.subscribe('userprofile.getById', this.params._id),
			Meteor.subscribe('user.getById', this.params._id),
...
Router.route('/profile/update', {
	...
	waitOn: function () {
		...
		return [
			Meteor.subscribe('userprofile.getByUserId', Meteor.userId()),
			Meteor.subscribe('tos.getForCurrentUser'),
		];
	}
});
Router.route('/profile/update/:_id', {
	name: 'profile.updateWithId',
	template: 'update',
	data: function () {
		if (!this.ready()) { return; }
...
Router.route('/profile/list', {
	...
	waitOn: function () {
		...
		return [
			Meteor.subscribe('userprofile.getTabular'),
			Meteor.subscribe('tos.getForCurrentUser'),
		];
	}
});
